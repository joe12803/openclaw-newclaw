...
# 2. 检查压缩包
BACKUP_FILE="/workspace/backups/latest_backup.tar.gz"
...
if [ -f "$BACKUP_FILE" ]; then
    ...
    cp -r $TEMP_RESTORE/memory/* /workspace/memory/ 2>/dev/null || true
    cp -r $TEMP_RESTORE/skills/* /workspace/skills/ 2>/dev/null || true

    echo "✅ 全量恢复成功！"
else
    echo "⚠️ 未找到最新的备份压缩包 (backups/latest_backup.tar.gz)。"
fi
